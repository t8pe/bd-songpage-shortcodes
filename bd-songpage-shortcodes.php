...
// This does the real work of returning the shortcode.
  return $mp3j_info .  mp3j_put('[mp3j track="' . get_post_meta( $song, "mp3ee", true ) . '" title="" style="bigger2"]' ) . $song_title . get_the_title( $song ) . $price . $product->regular_price . $buy_now . '<a href=' . get_site_url() . '?add-to-cart=' .  $song . $more_info . get_site_url() . "/song_wiki/" . get_the_title( $song ) . '">' . $end_string;
}
